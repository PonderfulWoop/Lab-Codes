...
int main(void){
	...
	//Copy A and B to  their buffers:
	ret = clEnqueueWriteBuffer(cmdq, abuf, CL_TRUE, 0, LIST_SIZE*sizeof(int), A, 0, NULL, NULL);
	ret = clEnqueueWriteBuffer(cmdq, bbuf, CL_TRUE, 0, LIST_SIZE*sizeof(int), B, 0, NULL, NULL);

	//Create prog from kernel source
	...
	cl_kernel kernel = clCreateKernel(prog, "vecadd", &ret);
	...
	ret = clEnqueueReadBuffer(cmdq, cbuf, CL_TRUE, 0, LIST_SIZE*sizeof(int), C, 0, NULL, NULL);
